 ...
 * configured with a genuinely non-blank value (see
 * ClaudeApiKeyConfiguredCondition - a blank value must behave identically to
 * an absent one). Absent (the default for every existing test and any
 ...
 * RazorpayAutoConfiguration's structure, down to the @Primary-over-coexistence
 * choice: SimulatedDiagnosisProvider still exists (harmlessly unused) rather
 * than being conditioned out.
 */
@Configuration
@Conditional(ClaudeApiKeyConfiguredCondition.class)
